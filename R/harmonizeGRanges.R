...
harmonizeGRanges=function(myGRList) {
  ...
  ALL_REGIONS=ALL_REGIONS[apply(do.call(cbind, lapply(myGRList, function(x) countOverlaps(ALL_REGIONS, x)==1)), 1, all)] # Only keep regions covered by all samples (+remove intra-sample overlaps)
  myGRList=lapply(myGRList, function(x) pintersect(findOverlapPairs(x, ALL_REGIONS))) # Get GRanges information for those regions
  myGRList=lapply(myGRList, function(x) {names(x)=1:length(x); return(x)}) # Reset names as some regions could have been discarded
  stopifnot(sapply(2:length(myGRList), function(x) identical(granges(myGRList[[1]]), granges(myGRList[[x]])))) # Make sure regions are strictly identical
  return(myGRList)
}