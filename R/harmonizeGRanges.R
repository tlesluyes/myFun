...
harmonizeGRanges=function(myGRList, cores=1) {
  ...
  myGRList=foreach::foreach(x=myGRList) %dopar% {GenomicRanges::pintersect(IRanges::findOverlapPairs(x, ALL_REGIONS))} # Get GRanges information for those regions
  myGRList=foreach::foreach(x=myGRList) %dopar% {names(x)=1:length(x); return(x)} # Reset names as some regions could have been discarded
  stopifnot(all(foreach::foreach(x=2:length(myGRList), .combine=c) %dopar% {identical(GenomicRanges::granges(myGRList[[1]]), GenomicRanges::granges(myGRList[[x]]))})) # Make sure regions are strictly identical
  return(myGRList)
}