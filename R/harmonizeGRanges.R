...
harmonizeGRanges=function(myGRList, cores=1) {
  ...
  myGRList=foreach(x=myGRList) %dopar% {pintersect(findOverlapPairs(x, ALL_REGIONS))} # Get GRanges information for those regions
  myGRList=foreach(x=myGRList) %dopar% {names(x)=1:length(x); return(x)} # Reset names as some regions could have been discarded
